<template>
	<div style="min-width: 350px;" class="task-list">
		<div v-if="pageSectionStore && pageSectionStore.displayedPageSections" ref="listArea" class="list-area p-0" style="min-height: 150px" :page="page.id">
			<draggable
        		...
        		@end="onDragEnd"
			>
				<template #item="{ pageSection, index }">
					<PageSection :index="index" :page="page" :pageSection="pageSection" :onPageSectionSubmit="onPageSectionSubmit" :onPageSectionDelete="onPageSectionDelete" />
				</template>
			</draggable>
		</div>
		<div v-else>
